...
def _coerce_numeric_strings(obj):
    """
    Recursively walk a parsed YAML structure and convert any string values
    that look like ints or floats (including scientific notation like '1e-4')
    to their actual numeric types.

    PyYAML versions differ in whether they parse '1e-4' as float or str.
    This normalises the output so callers always receive Python numbers.
    """
    ...
    if isinstance(obj, str):
        # Try int first (no dot or e), then float
        try:
            return int(obj)
        except ValueError:
            pass
...
def read_config(config_file):
    ...
    # Coerce any string-encoded numbers (e.g. '1e-4') to actual Python numeric
    # types. Some PyYAML versions (especially on HPC clusters) parse scientific
    # notation as strings rather than floats.
    cfg = _coerce_numeric_strings(cfg)
...
def prepare_dataset_new(data, batch_size=16):
    from dataset.weather_graph_dataset import WeatherGraphDatasetNew
...
def prepare_homogeneous_dataset(data, batch_size=16):
    from dataset.weather_graph_dataset import HomogeneousWeatherGraphDatasetNew
...
def prepare_homogeneous_inductive_dataset(
    ...
    from dataset.weather_graph_dataset import HomogeneousWeatherGraphDatasetInductive
    """
