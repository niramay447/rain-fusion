...
# ─────────────────────────────────────────────────────────────────────────────
# WAGGA WAGGA variant of satellite/au_preprocessor.py.
# Identical logic to the Sydney satellite preprocessor — only the crop indices
# differ (recomputed for the Wagga rural bbox on the fixed Himawari grid), plus
# a `stride` to keep the node count tractable over the larger region. The
# Himawari projection is global/fixed, so it is unchanged. Sydney file untouched;
# train.py imports one or the other by config. Timezone / start-of-period
# labelling are unchanged.
# ─────────────────────────────────────────────────────────────────────────────

# Geostationary projection for Himawari-8/9 centred at 140.7°E (global, fixed)
_PROJ = Proj(
    proj='geos', lon_0=140.7, h=35785863,
    x_0=0, y_0=0, a=6378137, b=6356752.3, units='m',
)

# Wagga bbox row/col indices on the 5500×5500 geostationary grid (2 km pixels).
# Computed analytically from the Wagga gauge bbox (+0.1° margin); validated by
# reproducing Sydney's known crop with the same logic.
_R0, _R1 = 4482, 4559
_C0, _C1 = 2999, 3106
...
def _read_accum(path: Path, stride: int) -> tuple[np.ndarray, np.ndarray] | None:
    """
    Read crrph_accum crop (subsampled by `stride`) from one file.
    ...
    with nc4.Dataset(path) as ds:
        raw = np.array(
            ds.variables['crrph_accum'][_R0:_R1 + 1:stride, _C0:_C1 + 1:stride],
            dtype=np.float32,
        )
...
class AUSatellitePreprocessor:
    ...
    accumulations over the Wagga rural domain.
    ...
    START of that window (H-1) so the convention matches the gauge.
    ...
    stride    : spatial subsampling on the 2-km grid.
                stride=3 → ~6 km, ~936 nodes (comparable to Sydney's 726).
                stride=2 → ~4 km, ~2106 nodes; stride=1 → 2 km, ~8424 nodes.
    """
    ...
    def _compute_coords(self) -> tuple[np.ndarray, np.ndarray]:
        """Compute flat (n_nodes,) lat/lon for the (strided) Wagga crop."""
        ...
        x_crop = nx_full[_C0:_C1 + 1:self.stride]
        y_crop = ny_full[_R0:_R1 + 1:self.stride]
    ...
    def load_day_hourly(self, date: datetime) -> pd.DataFrame:
        ...
        for h in range(1, 24):
            ...
            result = _read_accum(_file_path(self.base_path, dt), self.stride)
        ...
        result = _read_accum(_file_path(self.base_path, dt_mid), self.stride)
        if result is not None:
            accum, valid = result
        else:
            accum, valid = zero_accum, zero_valid
    ...
    def build_dataset(self, year: int) -> pd.DataFrame:
        ...
        print(
            f"Satellite CRRPH (Wagga) / {year}: {len(days)} days, "
            f"grid={self.grid_shape}, n_nodes={self.n_nodes}, stride={self.stride}"
        )
        dfs = []
